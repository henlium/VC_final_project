...
void decode()
{
  ...
  float *last_float_H = (float*)malloc(sizeof(float)*SIZE);
  float *last_float_L = (float*)malloc(sizeof(float)*SIZE);
  float *next_float = (float*)malloc(sizeof(float)*SIZE);
  float *next_float_H = (float*)malloc(sizeof(float)*SIZE);
  float *next_float_L = (float*)malloc(sizeof(float)*SIZE);
  ...
  DownSample(last_float, cur_float, width, height);
  interpolate(cur_float, last_float_L, width, height);
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      int pos = i*width+j;
      last_float_H[pos] = last_float[pos] - last_float_L[pos];
    }
  }
  ...
  while (true) {
    ...
    DownSample(next_float, cur_float, width, height);
    interpolate(cur_float, next_float_L, width, height);
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        int pos = i*width+j;
        next_float_H[pos] = next_float[pos] - next_float_L[pos];
      }
    }
